...
export function AuthProvider({
  ...
  // Auto-set orgId for Lovable/localhost environments without waiting for route guards
  useEffect(() => {
    const hostname = window.location.hostname;
    const isLocalOrLovable = hostname === 'localhost' ||
      hostname.includes('.lovable.app') ||
      hostname.includes('.lovableproject.com') ||
      hostname.includes('lovable.dev');

    if (isLocalOrLovable && !orgId && authConfig?.appInfo?.devOrgId) {
      setOrgId(authConfig.appInfo.devOrgId);
    }
  }, [authConfig, orgId]);
