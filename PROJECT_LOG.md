...
#### 🎨 BRAND MARKS + LINK PREVIEW — Session 26
Both predated the finished site (an indigo-on-cream placeholder card, and a stale favicon).

**Favicon — a bare Tanker "R", no background.** Generated by **`scripts/brand-icons.py`**
(fontTools + brotli + Pillow, dev-only) into three files, because a background-less mark has to
survive both light and dark browser chrome and each format solves that differently:
| file | how it handles the tab strip |
|---|---|
| `src/app/icon.svg` | **ADAPTS** — an SVG favicon is a live document, so a `prefers-color-scheme` block inside it recolours the glyph: near-black on light, white on dark. This is what modern browsers use. |
| `src/app/favicon.ico` | can't adapt → near-black. Only reached by browsers too old for SVG favicons (pre-16.4 Safari), which default to light chrome. |
| `src/app/apple-icon.png` | white on transparent. **iOS flattens icon transparency to BLACK**, so it lands as a white R on near-black — the site's own canvas. |
- ⚠️ **The SVG embeds the R as a PATH, not `<text>`.** A favicon is rendered with no access to our
  webfont, so `font-family="Tanker"` would silently fall back to something generic.

**OG card — a restaging of the hero** (`src/app/opengraph-image.tsx`): near-black canvas, the
tunnel's signal-orange/ember blooms and blue drift, square motes, and the centred Tanker lockup —
RIDDHIMAN DEB over PRODUCT—DESIGN BUILDER. The real hero is WebGL and Satori draws CSS, so it is
rebuilt rather than screenshotted.
- ⚠️⚠️ **SATORI CANNOT READ WOFF2** (ttf/otf/woff only), and Tanker ships as WOFF2. With no usable
  font it silently falls back to a generic sans and the card stops looking like the site.
  **`src/app/fonts/Tanker-Regular.ttf` is a committed conversion** — the woff2 is still what
  `next/font/local` uses; do not delete either. Regenerate per the header of `brand-icons.py`.
- ⚠️ **Satori has no `filter: blur()`** — every glow is a radial-gradient, soft by construction.
- ⚠️ **Satori ignores the `inset` shorthand.** `inset: 0` left the lockup with no resolved size, so
  it collapsed to its content in the top-left instead of centring. Write left/top/right/bottom out.
- ⚠️ **EVERY GLOW IS A FULL-BLEED BOX** the size of the whole card, with the bloom placed by
  percentage and given an explicit `circle <px>` radius. Small per-glow boxes showed straight clipped
  edges, worst on the ones hanging off-canvas on negative offsets. A three-variant probe render
  proved the gradient itself was always clean — the box geometry was the bug.
- `layout.tsx` now sets explicit `openGraph`/`twitter` title + description. The tab title is the
  brand ("Rudyman"), which is too thin alone in a shared link.
